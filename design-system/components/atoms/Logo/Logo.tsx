...
const dotRevealVariants: Variants = {
...
const dotIdleVariants: Variants = {
  idle: {
      ...
      {isLoader ? (
        ...
        <>
          ...
          <motion.circle
            ...
            variants={isIdle && !isReveal ? dotIdleVariants : isReveal ? dotRevealVariants : undefined}
            initial={isReveal ? 'hidden' : undefined}
            animate={
              isReveal ? 'show' :
              isIdle   ? 'idle' :
...
export function CortexLogo({
  ...
  const textY     = Math.round(iconSize * 0.69)
  const totalW    = textX + Math.round(fontSize * 3.5)
  const isReveal  = mode === 'reveal' && !reduced
